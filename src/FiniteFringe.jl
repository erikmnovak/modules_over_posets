...
struct FringeModule{K}
    ...
    phi::SparseMatrixCSC{K,Int}       # size |D| x |U|

    function FringeModule{K}(P::FinitePoset,
                             U::Vector{Upset},
                             D::Vector{Downset},
                             phi::SparseMatrixCSC{K,Int}) where {K}
        M = new{K}(P, U, D, phi)
...
# Convenience constructor that infers K from the SparseMatrix element type.
FringeModule(P::FinitePoset,
             U::Vector{Upset},
             D::Vector{Downset},
             phi::SparseMatrixCSC{K,Int}) where {K} = FringeModule{K}(P, U, D, phi)
