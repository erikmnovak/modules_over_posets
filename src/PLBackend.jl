...
function encode_fringe_boxes(Ups::Vector{BoxUpset},
    ...
    # enumerate all cells, but stop if we exceed max_regions
    using Base.Iterators: product
    axes = [collect(0:length(coords[i])) for i in 1:n]
    cells_iter = product((axes[i] for i in 1:n)...)
    ...
    for t in cells_iter
        ...
        push!(sig_y, y); push!(sig_z, z); push!(reps, x)
        count += 1
        if count >= max_regions
            @warn "encode_fringe_boxes: reached max_regions cap; stopping early."
            break
        end
    end

    # 3) collapse equivalent signatures (use content-hashable keys)
    # Use content-hashable BitVector keys so equal (y,z) signatures dedupe correctly.
    seen = Dict{Tuple{BitVector,BitVector},Int}()
    sigY = BitVector[]; sigZ = BitVector[]; Reps = Vector{Float64}[]
    for k in 1:length(sig_y)
        # sig_y[k] and sig_z[k] are already BitVector; use them *directly* as the key.
        key = (sig_y[k], sig_z[k])
        if !haskey(seen, key)
            seen[key] = 1
            push!(sigY, sig_y[k]); push!(sigZ, sig_z[k]); push!(Reps, reps[k])
        end
    end
