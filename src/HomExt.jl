...
const _MaskKey = Tuple{Vararg{Bool}}
const _CompKey = Tuple{_MaskKey,_MaskKey}

mutable struct CompCache
    P::FinitePoset
    adj::Vector{Vector{Int}}
    # Cache components of U cap D.
    # IMPORTANT: key by masks (by value), not by local indices, since indices
    # are reused across resolution degrees.
    table::Dict{_CompKey, Tuple{Vector{Int},Int}}  # (Tuple(U.mask), Tuple(D.mask)) -> (comp_id, ncomp)
end
CompCache(P::FinitePoset) =
    CompCache(P, _hasse_undirected(P), Dict{_CompKey,Tuple{Vector{Int},Int}}())


"Connected components for a subset mask given an undirected adjacency list."
function _components_of_mask(adj::Vector{Vector{Int}}, mask::BitVector)
    comp = fill(0, length(mask))
    cid = 0
    for v in 1:length(mask)
        if mask[v] && comp[v] == 0
            cid += 1
            stack = [v]; comp[v] = cid
            while !isempty(stack)
